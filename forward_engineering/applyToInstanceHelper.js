...
const applyToInstance = async (connectionInfo, logger, app) => {
	...
	try {
		const queries = joinFunctionBodies(splitUpDropCreateStatements(connectionInfo.script.split('\n\n')))
			.map(query => {
				return removeDelimiter(_.trim(query));
			})
			.filter(q =>
				[Boolean, q => !q.startsWith('USE'), q => !(q.startsWith('/*') && q.endsWith('*/'))].every(f => f(q)),
			);
		...
		await async.mapSeries(queries, async query => {
			...
			try {
				...
				logger.log(
					'error',
					{ message: `query ${query} failed with: ${e.message}`, error: e },
					'Apply to instance',
				);
